...
# initlalze MPU 6050
#i2cMPU = machine.I2C(1, sda=sda_pin, scl=scl_pin, freq=400000)
#mpu = MPU6050(i2cMPU)
...
time.sleep(2) # wait for QMI initialize
...
while True:
    ...
    #print('Pitch (Blue) :',acc[0], 'Roll (orange) : ', acc[1])
    
    #cntr_st(str(acc[0]),68,1,255,255,255) 
    #cntr_st(str(acc[1]),88,1,255,255,255)
    #print(acc[0], acc[1], acc[2]
    
    # clear screen every refresh time
    circle(xc,yc,102,colour(0,0,0))
    #circle(xc,yc,120,colour(0,0,0))
       
    # Calculate pitch angle (in degrees)
    #pitch = math.atan2(acc[0], math.sqrt(acc[1] ** 2 + acc[2] ** 2)) * 180 / math.pi
    '''pitch = math.atan2(-1*acc[0], math.sqrt(acc[1] ** 2 + acc[2] ** 2)) * 180 / math.pi
    # Calculate roll angle (in degrees)
    
    roll = math.atan2(acc[1], math.sqrt(acc[0] ** 2 + acc[2] ** 2)) * 180 / math.pi
    #roll = math.atan2(-1*acc[1], acc[2]) * 180 / math.pi
    
    cntr_st(str(pitch),88,3,255,255,255) 
    cntr_st(str(roll),128,3,255,255,255)'''
       
    # draw crosshair
    c = colour(255,255,255)
    LCD.line(120,0,120,240,c)
    LCD.line(0,120,240,120,c)
    ...
    pitch = math.atan(zAccel/yAccel)
    pitchDeg = pitch/2/math.pi*360
    roundpitchDeg = round(pitchDeg,2)
    mapped_roundpitchDeg = int((roundpitchDeg / 90) * 240)
    
    dispOffset = 70
    if mapped_roundpitchDeg > dispOffset :
        mapped_roundpitchDeg = dispOffset
    if mapped_roundpitchDeg < -1*dispOffset :
        mapped_roundpitchDeg = -1*dispOffset
                       
    roll = math.atan(xAccel/yAccel)
    rollDeg = roll/2/math.pi*360
    roundrollDeg = round(rollDeg,2)
    mapped_roundrollDeg = int((roundrollDeg / 90) * 240)
    
    if mapped_roundrollDeg > dispOffset :
        mapped_roundrollDeg = dispOffset
    if mapped_roundrollDeg < -1*dispOffset :
        mapped_roundrollDeg = -1*dispOffset
    
    pitchX = 120 + mapped_roundpitchDeg
    rollY = 120   + mapped_roundrollDeg
    
    # draw ciecle marker
    markerOffset = 0
    #circle(pitchX-markerOffset,rollY-markerOffset,10,colour(255,0,0))
    circle(rollY-markerOffset,pitchX-markerOffset,10,colour(255,0,0))
    
    #cntr_st(str(roundpitchDeg),88,3,255,255,255) 
    #cntr_st(str(roundrollDeg),128,3,255,255,255)
      
    LCD.show()
    # Update screen
    #time.sleep(0.05)
    #Short delay
    #print("Pitch: ",mapped_roundpitchDeg, " Roll: ",mapped_roundrollDeg)
    #print("Pitch: ",pitchX, " Roll: ",rollY)
